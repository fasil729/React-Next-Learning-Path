...
const JobDescription = ({ params }: { params: { id: string } }) => {
  ...
  const formattedStartDate = dayjs(opportunity?.data.startDate).format(
    "MMM D, YYYY"
  );
  const formattedEndDate = dayjs(opportunity?.data.endDate).format(
    "MMM D, YYYY"
  );
  const formattedPostedOn = dayjs(opportunity?.data.datePosted).format(
    "MMM D, YYYY"
  );
  const formattedDeadline = dayjs(opportunity?.data.deadline).format(
    "MMM D, YYYY"
  );
          ...
          <li className="flex items-center gap-4">
            ...
            <div className="font-epilogue font-normal text-base leading-[25.6px] text-[#25324B]">
              ...
              <p className="font-semibold">
                {opportunity?.data.location.join(", ")}
              </p>
            </div>
          </li>
          <li className="flex items-center gap-4">
